...
import { Radio, Info, Search, RefreshCw, Filter } from "lucide-react";
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-text-muted">
            {totalCount} station{totalCount !== 1 ? 's' : ''} assigned
          </p>
        </div>
        <Button 
          variant="secondary" 
          ...
          <div className="hidden lg:block">
            <DataTable 
              ...
              onRowClick={(row: Station) => {
                const data = getStationData(row);
                if (data) router.push(`/vendor/stations/${data.id}`);
              }}
              pagination={true}
              pageSize={10}
            />
          ...
          <div className="lg:hidden space-y-4 p-4">
            {stations.map((station) => {
              ...
              return (
                <div
                  key={data.id}
                  onClick={() => router.push(`/vendor/stations/${data.id}`)}
